...
UFlightPathBatchComponent::UFlightPathBatchComponent()
	: bQueuedRedraw(false)
{
}

int32 UFlightPathBatchComponent::AddPath(const FFlightNavigationPath& NavPath, const FVector& PathOffset,
	const FColor PathColor, const int32 Index)
{
	...
	ConvertNavPathPointsToVector(NavPath.GetPathPoints(), PathPoints);
...
void UFlightPathBatchComponent::RemovePath(const int32 Index)
{
...
void UFlightPathBatchComponent::ApplyWorldOffset(const FVector& InOffset, bool bWorldShift)
{
...
void UFlightPathBatchComponent::Redraw()
{
	if (!bQueuedRedraw)
	{
		bQueuedRedraw = true;
...
void UFlightPathBatchComponent::DrawPath(const FBatchedPath& BatchedPath)
{
...
void UFlightPathBatchComponent::ConvertNavPathPointsToVector(const TArray<FNavPathPoint>& In, TArray<FVector>& Out)
{
	Out.Reset(In.Num());
